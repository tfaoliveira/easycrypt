...
and cbv (st : state) (s : subst) (f : form) (args : args) : form =
  ...
  | FespF ef ->
    ...
    assert (not (Subst.has_mem s mleft));
    assert (not (Subst.has_mem s mright));
    let espf_pr = pair_map (norm st s) ef.espf_pr in
    let espf_po = pair_map (norm st s) ef.espf_po in
    let espf_fl = norm_xfun st s ef.espf_fl in
    let espf_fr = norm_xfun st s ef.espf_fr in
    let espf_f  = norm st s ef.espf_f in
    f_espF_r {espf_pr; espf_fl; espf_fr; espf_po; espf_f }

  | FespS es ->
    assert (is_Aempty args);
    assert (not (Subst.has_mem s (fst es.esps_ml)));
    assert (not (Subst.has_mem s (fst es.esps_mr)));
    let esps_pr = pair_map (norm st s) es.esps_pr in
    let esps_po = pair_map (norm st s) es.esps_po in
    let esps_sl = norm_stmt s es.esps_sl in
    let esps_sr = norm_stmt s es.esps_sr in
    let esps_ml = norm_me s es.esps_ml in
    let esps_mr = norm_me s es.esps_mr in
    let esps_f  = norm st s es.esps_f in
