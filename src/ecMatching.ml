        ...
        let subp =
          ...
          | FcHoareF chf ->
              let subce_pr, subce_po =
                ...
                (Ctxt.add_memory mpr ce, Ctxt.add_memory mpo ce) in
              let calls =
                List.map (fun (_, cb) -> (ce, cb.cb_called))
                  (EcPath.Mx.bindings chf.chf_co.c_calls) in
              doit pos (`WithSubCtxt ((subce_pr, chf.chf_pr)        ::
                                      (subce_po, chf.chf_po)        ::
                                      (ce      , chf.chf_co.c_self) :: calls))
          ...
          | Fcoe coe ->
            let subctxt = Ctxt.add_memory coe.coe_mem ce in
