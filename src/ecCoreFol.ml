...
(* -------------------------------------------------------------------- *)
let a_labels_is_left (a : (EcIdent.t * EcIdent.t * form) list) (l : EcIdent.t) =
  List.exists (fun (l', _, _) -> (EcIdent.name l = EcIdent.name l')) a

let a_labels_is_right (a : (EcIdent.t * EcIdent.t * form) list) (l : EcIdent.t) =
  List.exists (fun (_, l', _) -> (EcIdent.name l = EcIdent.name l')) a

let a_labels_partition_left (b : EcIdent.t -> bool) (a : (EcIdent.t * EcIdent.t * form) list) =
  List.partition (fun (l, _, _) -> b l) a

let a_labels_partition_right (b : EcIdent.t -> bool) (a : (EcIdent.t * EcIdent.t * form) list) =
  List.partition (fun (_, l, _) -> b l) a

let a_labels_partition (b : EcIdent.t -> EcIdent.t -> bool) (a : (EcIdent.t * EcIdent.t * form) list) =
  List.partition (fun (l1, l2, _) -> b l1 l2) a
...
let a_labels_is_disjoint (ls_l : unit EcMaps.Mstr.t) (ls_r : unit EcMaps.Mstr.t) (a : (EcIdent.t * EcIdent.t * form) list) =
