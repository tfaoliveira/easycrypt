...
let f_map gt g fp =
  ...
  | FeHoareF hf ->
      let ehf_pr = g hf.ehf_pr  in
      let ehf_po = g hf.ehf_po  in
        FSmart.f_eHoareF (fp, hf)
          { hf with ehf_pr; ehf_po }

  | FeHoareS hs ->
      let ehs_pr = g hs.ehs_pr  in
      let ehs_po = g hs.ehs_po  in
        FSmart.f_eHoareS (fp, hs)
          { hs with ehs_pr; ehs_po; }
