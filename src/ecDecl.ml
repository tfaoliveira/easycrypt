...
type ax_schema = {
  ...
  axs_eparams : sc_eparams; (* variables representing expression *)
  axs_loca    : locality;
...
let sc_instantiate
    ...
    ty_args memtype sc_args f =
  let fs = EcTypes.Tvar.init (List.map fst ty_params) ty_args in
  let sty = { ty_subst_id with ts_v = fs;
                               ts_memtype = Some memtype } in
  ...
  let tx f_old f_new = match f_old.f_node, f_new.f_node with
    | Fcoe coe_old, Fcoe coe_new
      when EcTypes.is_schema (snd coe_old.coe_mem) ->
      let fs =
        List.fold_left (fun s (id,e) ->
            let f = EcCoreFol.form_of_expr ~mem:(EcCoreFol.f_mem coe_new.coe_mem) e in
  ...
  let fs = Fsubst.f_subst_init ~sty ~esloc:mexpr () in
