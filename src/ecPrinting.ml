...
(* -------------------------------------------------------------------- *)
let pp_espF (ppe : PPEnv.t) ?prpo fmt esp =
  let ppe =
    PPEnv.create_and_push_mems
      ppe [(EcFol.mleft , esp.espf_fl); (EcFol.mright, esp.espf_fr)]
  in
  ...
  Format.fprintf fmt "@\n%!";
  Format.fprintf fmt "%a@\n%!" (pp_pre ppe ?prpo) (fst esp.espf_pr);
  Format.fprintf fmt "    %a ~ %a@\n%!"
    (pp_funname ppe) esp.espf_fl
    (pp_funname ppe) esp.espf_fr;
  Format.fprintf fmt "@\n%a%!" (pp_post ppe ?prpo) (fst esp.espf_po)

(* -------------------------------------------------------------------- *)
let pp_espS (ppe : PPEnv.t) ?prpo fmt esp =
  let ppe = PPEnv.push_mems ppe [esp.esps_ml; esp.esps_mr] in
  ...
  let ppnode = c_ppnode ~width:40 ~mem:(fst esp.esps_ml, fst esp.esps_mr)
    ...
    Format.fprintf fmt "(d-tx   ) : %a@\n%!" (pp_form ppe) esp.esps_f;
    Format.fprintf fmt "@\n%!";
    Format.fprintf fmt "%a%!" (pp_pre ppe ?prpo) (fst esp.esps_pr);
    Format.fprintf fmt "@\n%!";
    Format.fprintf fmt "%a" (pp_node `Both) ppnode;
    Format.fprintf fmt "@\n%!";
    Format.fprintf fmt "%a%!" (pp_post ppe ?prpo) (fst esp.esps_po)
...
module PPGoal = struct
  ...
  let pp_goal1 ?(pphyps = true) ?prpo ?(idx) (ppe : PPEnv.t) fmt (hyps, concl) =
    ...
    | FespS   esp  -> pp_espS     ?prpo ppe fmt esp
