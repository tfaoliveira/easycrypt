...
let rec trans_genlvalue (ue : EcUnify.unienv) (env : EcEnv.env) (plvalue : plvalue) =
  ...
  | PLvProji (lv, i) -> begin
     ...
     let uidmap = UE.assubst ue in
     let tyinst = ty_subst (Tuni.subst uidmap) ty in

     match (EcEnv.ty_hnorm tyinst env).ty_node with
     | Ttuple tys ->
...
and trans_qrefs
    ...
    (ue     : EcUnify.unienv)
    (env    : EcEnv.env)
    (pqrefs : (plvalue list) located)
  =
  let qrefs = List.map (trans_qref ~disjoint:false ue env) (unloc pqrefs) in
...
and trans_qrref ~disjoint (ue : EcUnify.unienv) (env : EcEnv.env) (pqrref : pqrref) =
  let for1 (qr, x) =
    let qr, ty = trans_qref ~disjoint:false ue env qr in
    let x =
  ...
  let qrref, bds =
    let qrs = List.map proj3_1 qrref in
...
and check_qref_disjoint env loc qref =
...
let transcall
  ...
  let cargs =
    let targ  = ty_subst (Tuni.subst (UE.assubst ue)) fsig.fs_arg in

    match args.fa_classical with
    | [] ->
    ...
    | [_] as args -> process_cargs [targ] args
    ...
    | args ->
      let lty =
        ...
        | Ttuple lty -> lty
        | _ -> [targ] in
      process_cargs lty args
  in

  let qargs =
    let tqarg = ty_subst (Tuni.subst (UE.assubst ue)) fsig.fs_qarg in

    match args.fa_quantum with
    | [] ->
        if not (EcReduction.EqTest.for_type env tqarg tunit) then
          tyerror loc env (InvalidFunAppl FAE_WrongArgCount);
        quantum_unit

    | _ when EcReduction.EqTest.for_type env tqarg tunit ->
        tyerror loc env (InvalidFunAppl FAE_WrongArgCount);

    | [pqr] ->
       let qr, ty = trans_qref ~disjoint:true ue env pqr in
       unify_or_fail env ue (EcLocation.loc pqr) ~expct:tqarg ty; qr

    | args ->
      let lty =
        match (EcEnv.Ty.hnorm tqarg env).ty_node with
        | Ttuple lty -> lty
        | _ -> [tqarg] in

      if List.length args <> List.length lty then
        tyerror loc env (InvalidFunAppl FAE_WrongArgCount);

      let qref =
        List.map2
          (fun qr ty ->
            let loc = qr.pl_loc in
            let qr, qrty = trans_qref ~disjoint:false ue env qr in
...
(* -------------------------------------------------------------------- *)
and transmodsig_body (env : EcEnv.env) (sa : Sm.t) (is : pmodule_sig_struct_body) =
  let transsig1 mr = function
    | `FunctionDecl f ->
      let name = f.pfd_name in

      let trans_args (q : quantum) (args : (osymbol * pty) list) =
        let tyargs =
          List.map
            (fun (x, ty) -> {
               ov_quantum = q;
               ov_name    = omap unloc x.pl_desc;
               ov_type    = transty_for_decl env ty
            })
            args
        in

        let anames = List.pmap (fun (x, _) -> unloc x) args  in

        anames, tyargs
      in

      let cnames, ctyargs = trans_args `Classical f.pfd_tyargs.fp_classical in
      let qnames, qtyargs = trans_args `Quantum f.pfd_tyargs.fp_quantum in

      oiter
        (fun (_, a) ->
          tyerror name.pl_loc env
            (InvalidModSig (MTS_DupArgName (unloc name, unloc a))))
        (Msym.odup unloc (cnames @ qnames));
      ...
      let sig_ = {
        ...
        fs_arg    = ttuple (List.map ov_type ctyargs);
        fs_qarg   = ttuple (List.map ov_type qtyargs);
        fs_anames = ctyargs;
        fs_qnames = qtyargs;
        fs_ret    = resty;
      } in
      ...
      let mr = EcModules.add_oinfo mr name.pl_desc oi in

      (mr, ([name], [Tys_function sig_]))

    | `Include (i,proc,restr) ->

      let (_modty, sig_) = transmodtype env i in

      if not (List.is_empty sig_.mis_params) then
        tyerror i.pl_loc env (InvalidModType MTE_IncludeFunctor);

      let check_xs xs =
        let for1 ({ pl_desc = name } as x) =
          let test (Tys_function fs) = sym_equal fs.fs_name name in
          if not (List.exists test sig_.mis_body) then
            let modsymb = fst (unloc i) @ [snd (unloc i)] in
            tyerror (loc x) env (UnknownFunName (modsymb, name))
        in List.iter for1 xs in

      let in_xs xs (Tys_function fs) =
      ...
      let update_mr mr (Tys_function fs) =
        let name = mk_loc (loc i) fs.fs_name in
        let mr   = EcModules.change_oicalls mr fs.fs_name calls in
        (mr, name)
      in
      ...
      let filter p =
        let for1 mr item =
          if p item then
            let mr, name = update_mr mr item in
            Some (mr, (name, item))
          else None
        in List.fold_left_pmap for1 mr sig_.mis_body in
      ...
      let mr, nmbd =
        ...
        | Some (`MInclude xs) ->
          check_xs xs; filter (in_xs xs)

        | Some (`MExclude xs) ->
          check_xs xs; filter (fun fs -> not (in_xs xs fs)) in

      (mr, List.split nmbd) in

  let names, items, mr =
    let mr, nmit = List.fold_left_map transsig1 EcModules.mr_empty is in
    let names, items = List.split nmit in
    List.flatten names, List.flatten items, mr in
...
and transstruct1 (env : EcEnv.env) (st : pstructure_item located) =
  ...
  | Pst_fun (decl, body) -> begin
      ...
      let params (q : quantum) (params : (osymbol * pty) list) =
        let params1 (s, pty) =
          let var = {
            v_quantum = q;
            v_name    = check_name s;
            v_type    = transty tp_uni env ue pty
          } in (var, loc s) in

        List.map params1 params in
      ...
      let cparams = List.map (fundef_check_decl (ty_subst ts) env) cparams in
      let qparams = List.map (fundef_check_decl (ty_subst ts) env) qparams in
      ...
      let fun_ =
        { f_name   = decl.pfd_name.pl_desc;
          f_sig    = {
            ...
            fs_arg    = ttuple (List.map (fun vd -> vd.v_type) cparams);
            fs_qarg   = ttuple (List.map (fun vd -> vd.v_type) qparams);
            fs_anames = List.map ovar_of_var cparams;
            fs_qnames = List.map ovar_of_var qparams;
            fs_ret    = retty;
          };
          f_def = FBdef {
            f_locals = locals;
      ...
      in [], [(decl.pfd_name.pl_desc, MI_Function fun_)]
    end
  ...
  let add_local memenv local =
    ...
    let xs =
      List.map2
        (fun x ty ->
          let var = {
            v_quantum = local.pfl_quantum;
            v_name    = x.pl_desc;
            v_type    = ty;
          } in (var, x.pl_loc)) xs xsvars in
...
=
  let transcall (name : pgamepath) (args : pexpr funargs) =
  ...
  | PSmeasure (plv, pqref, pmeas) ->
     ...
     let qref, bds = trans_qrref ~disjoint:true ue env pqref in
...
and trans_form_or_pattern env ?mv ?ps ue pf tt =
  ...
  let rec transf_r opsc env f =
    ...
    | PFprob (gp, args, m, event) ->
        ...
        let (args, _), _ =
          let args = { fa_classical = args; fa_quantum = [] } in
          transcall
    ...
    | PFequivF (pre, (gp1, gp2), post) ->
        ...
        let pre'  = trans_ec opsc penv pre in
        let post' = trans_ec opsc qenv post in
  ...
  and trans_ec opsc env (f, qeq) =
    let ec_f = transf_r opsc env f in
