...
let rec unify_instrs umap i1 i2 =
 match i1.i_node, i2.i_node with
  ...
  | Srnd(_, e1), Srnd(_, e2) ->
     unify_exprs umap e1 e2
  | Scall(_, _, args1,qarg1), Scall(_, _, args2,qarg2)
      when EcAst.is_quantum_unit qarg1 && EcAst.is_quantum_unit qarg2 ->
    List.fold_left
      (fun umap (e1, e2) -> unify_exprs umap e1 e2)
      umap (List.combine args1 args2)
