...
let t_rewrite_equiv side dir cp (equiv : equivF) equiv_pt rargslv tc =
  ...
  let prefix, (llv, func, largs, qarg), suffix =
  ...
  assert (is_quantum_unit qarg); (* FIXME: quantum *)
  ...
  let prog = s_call (rlv, new_func, rargs, quantum_unit) in
...
let process_rewrite_equiv info tc =
  ...
  let rargslv =
    ...
    | None -> None
    | Some ( { pl_loc = loc; pl_desc = pargs }, pres) ->
      (* FIXME: quantum *)
      let pargs = { fa_classical = pargs; fa_quantum = []; } in
        ...
        begin
          try
            ...
            let (args, _), ret_ty = EcTyping.trans_args subenv ue loc proc.f_sig pargs in
