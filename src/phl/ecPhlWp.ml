...
module TacInternal = struct
  ...
  let t_esp_wp ?(uselet=true) ij tc =
    ...
    let i = omap fst ij and j = omap snd ij in
    let s_hdl,s_wpl = o_split i es.esps_sl in
    let s_hdr,s_wpr = o_split j es.esps_sr in
    ...
    let s_wpr, post = wp ~uselet env memr s_wpr' post in
    check_wp_progress tc i es.esps_sl s_wpl;
    check_wp_progress tc j es.esps_sr s_wpr;
