...
(* -------------------------------------------------------------------- *)
let t_inline_esp_r ~use_tuple side sp tc =
  let equiv = tc1_as_espS tc in
  let concl =
    match side with
    | `Left  ->
        let (me, stmt) = LowInternal.inline ~use_tuple tc equiv.esps_ml sp equiv.esps_sl in
          f_espS_r { equiv with esps_ml = me; esps_sl = stmt; }
    | `Right ->
        let (me, stmt) = LowInternal.inline ~use_tuple tc equiv.esps_mr sp equiv.esps_sr in
...
let t_inline_esp     ~use_tuple = FApi.t_low2 "esp-inline"     (t_inline_esp_r ~use_tuple)
