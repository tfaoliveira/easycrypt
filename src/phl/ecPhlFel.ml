...
and callable_oracles_i env modv os i =
  match i.i_node with
    | Scall  (_, f, _, _) -> callable_oracles_f  env modv os f
    ...
    | Sasgn _ | Srnd _ | Sassert _ | Squantum _ | Smeasure _ -> os
...
let t_failure_event_r (at_pos, cntr, ash, q, f_event, pred_specs, inv) tc =
  ...
  let init_goal =
    ...
    let eqparams =
      ...
      let var_of_ovar ov = { v_quantum = `Classical; v_name = oget ov.ov_name; v_type = ov.ov_type } in
