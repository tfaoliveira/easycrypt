...
let alias_stmt env id (pf, _) me i =
  ...
  | Scall (Some lv, f, args, qr) when is_quantum_unit qr ->
    (* FIXME QUANTUM : is_quantum_unit qr needed ? *)
    let ty       = (EcEnv.Fun.by_xpath f env).f_sig.fs_ret in
    let (me, pv) = dopv ty in
    (me, [i_call (Some (LvVar (pv, ty)), f ,args, qr); i_asgn (lv, e_var pv ty)])
  | _ ->
...
let process_weakmem (side, id, params) tc =
  ...
  check `Classical tc f;        (* FIXME: quantum *)
  ...
  let process_decl (x, ty) =
    ...
    { ov_name = x; ov_type = ty; ov_quantum = `Classical }
  in
