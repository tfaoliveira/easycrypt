...
let is_alpha_eq hyps f1 f2 =
  ...
  let check_qr env s qr1 qr2 =
    let qr2 = EcFol.qr_subst s qr2 in
...
let rec simplify ri env f =
  ...
  match f.f_node with
  | FhoareF hf ->
      let hf_f = if ri.ri.modpath then EcEnv.NormMp.norm_xfun env hf.hf_f else hf.hf_f in
      f_map (fun ty -> ty) (simplify ri env) (f_hoareF_r { hf with hf_f })

  | FeHoareF hf ->
      let ehf_f = if ri.ri.modpath then EcEnv.NormMp.norm_xfun env hf.ehf_f else hf.ehf_f in
      f_map (fun ty -> ty) (simplify ri env) (f_eHoareF_r { hf with ehf_f })

  | FbdHoareF hf ->
      let bhf_f = if ri.ri.modpath then EcEnv.NormMp.norm_xfun env hf.bhf_f else hf.bhf_f in
  ...
  | FeagerF eg ->
      let eg_fl = if ri.ri.modpath then EcEnv.NormMp.norm_xfun env eg.eg_fl else eg.eg_fl in
      let eg_fr = if ri.ri.modpath then EcEnv.NormMp.norm_xfun env eg.eg_fr else eg.eg_fr in
      f_map (fun ty -> ty) (simplify ri env) (f_eagerF_r { eg with eg_fl ; eg_fr; })

  | Fpr pr when ri.ri.modpath ->
      let pr_fun = if ri.ri.modpath then EcEnv.NormMp.norm_xfun env pr.pr_fun else pr.pr_fun in
...
let zpop ri side f hd =
  ...
  | Zhl {f_node = FequivF hf}, [pr;po] ->
    f_qequivF_r {hf with ef_pr = { hf.ef_pr with ec_f = pr } ; ef_po = { hf.ef_po with ec_f = po } }
  | Zhl {f_node = FequivS hs}, [pr;po] ->
    f_qequivS_r {hs with es_pr = { hs.es_pr with ec_f = pr } ; es_po = {hs.es_po with ec_f = po } }
...
let rec conv ri env f1 f2 stk =
  ...
  | FequivS es1, FequivS es2
    ...
    | subst ->
      let subst = Fsubst.f_subst subst in
      conv ri env
        es1.es_pr.ec_f (subst es2.es_pr.ec_f)
        (zhl f1 [es1.es_po.ec_f] [subst es2.es_po.ec_f] stk)
    | exception NotConv -> force_head ri env f1 f2 stk
    end
