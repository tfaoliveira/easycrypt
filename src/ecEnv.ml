...
(*      if a <> [] then
    ...
    let update_id id mods =
      let update me =
        ...
        | ME_Decl (mt, mt_restr) ->
          ...
          { me with me_body = ME_Decl ( mt, mt_restr ) }
        | _ -> me
      in
      MMsym.map_at
        (List.map
  ...
  let me_of_mt env name ((mty, mr) : mty_mr) =
    let modsig =
      let modsig =
        match
          omap
            ...
            (MC.by_path
               ...
               (IPPath mty) env)
        with
        | None -> lookup_error (`Path mty)
        | Some x -> x
      in
      EcSubst.subst_modsig EcSubst.empty modsig.tms_sig
    in
  ...
  let bind_param id mty env =
    bind_local id (mty, mr_empty) env

  let bind_params params env =
    bind_locals (List.map (fun (id, mty) -> (id, (mty, mr_empty))) params) env
...
module NormMp = struct
  ...
  let get_restr_me env me mp =
    match me.me_body with
    | EcModules.ME_Decl (mt, mr) ->

      let ms = ModTy.by_path mt env in
      let oi = ms.tms_sig.mis_oinfos in
      mr, oi
    ...
    | _ ->
      ...
      let env = Mod.bind_params me.me_params env in
      ...
      let mr = { mr_xpaths = ur_xpaths;
                 mr_mpaths = ur_mpaths } in
      mr, oi
  ...
  let sig_of_mp env mp =
    ...
    let mr, oi = get_restr_me env me mp in
  ...
  (* -------------------------------------------------------------------- *)
  let mod_type_equiv env (mty1, mr1) (mty2, mr2) =
    (mty_equal mty1 mty2) &&
...
module LDecl = struct
  ...
  let ld_subst s ld =
    ...
    | LD_modty mtr->
        let p = Fsubst.mty_mr_subst s mtr
        in LD_modty p
