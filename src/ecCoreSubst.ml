...
(* ------------------------------------------------------------------ *)
let rec add_binding (s : f_subst) ((x, gty) as xt : binding) : f_subst * binding =
...
and add_bindings (s : f_subst) (bds : bindings) : f_subst * bindings =
  List.map_fold add_binding s bds

(* ------------------------------------------------------------------ *)
and mr_subst (s : f_subst) (mr : mod_restr) : mod_restr =
  { mr_mem = mer_subst s mr.mr_mem;
...
and mty_subst (s : f_subst) (mty : module_type) : module_type =
  let s, mt_params = mod_params_subst s mty.mt_params in
...
(* ------------------------------------------------------------------ *)
and mod_params_subst (s : f_subst) mt_params =
  let s, b =
    add_bindings
      s
...
(* ------------------------------------------------------------------ *)
and ff_subst (s : f_subst) ff =
  let s, ff_params = mod_params_subst s ff.ff_params in
  let ff_xp = x_subst s ff.ff_xp in
  { ff_params; ff_xp }

(* ------------------------------------------------------------------ *)
and mer_subst (s : f_subst) (mer : mem_restr) : mem_restr =
  match mer with
  | Empty | Quantum | Classical -> mer
  | Var (q, x) -> Var(q, x_subst s x)
  | GlobFun ff -> GlobFun (ff_subst s ff)
  | Union(s1,s2) -> Union(mer_subst s s1, mer_subst s s2)
  | Inter(s1,s2) -> Inter(mer_subst s s1, mer_subst s s2)
  | Diff(s1,s2)  -> Diff(mer_subst  s s1, mer_subst s s2)
...
(* -------------------------------------------------------------------- *)
let rec pvt_subst (s : f_subst) ((pv, ty) as p) : prog_var_ty =
  let pv' = pv_subst s pv in
  let ty' = ty_subst s ty in
...
and qr_subst (s : f_subst) = qr_map (pvt_subst s)

(* -------------------------------------------------------------------- *)
let rec s_subst_top (s : f_subst) : stmt -> stmt =
  let rec lv_subst (lv : lvalue) : lvalue =
    match lv with
    | LvVar pvt ->
      LvVar (pvt_subst s pvt)

    | LvTuple pvs ->
      LvTuple (List.Smart.map (pvt_subst s) pvs)

  and i_subst (i : instr) : instr =
    match i.i_node with
    | Squantum (qr, qop, e) ->
      i_quantum (qr_subst s qr, qop, e_subst s e)

    | Smeasure(lv, qr, e) ->
      i_measure (lv_subst lv, qr_subst s qr, e_subst s e)

    | Sasgn (lv, e) ->
      i_asgn (lv_subst lv, e_subst s e)

    | Srnd (lv, e) ->
      i_rnd (lv_subst lv, e_subst s e)

    | Scall (olv, mp, args, qarg) ->
      let olv'  = olv |> OSmart.omap lv_subst in
      let mp'   = EcPath.x_subst_abs s.fs_mod mp in
      let args' = List.Smart.map (e_subst s) args in
      let qarg' = qr_subst s qarg in
      i_call (olv', mp', args', qarg')

    | Sif (e, s1, s2) ->
      i_if (e_subst s e, s_subst s1, s_subst s2)

    | Swhile(e, b) ->
      i_while (e_subst s e, s_subst b)
    ...
    | Smatch (e, b) ->
      ...
      i_match (e_subst s e, List.Smart.map forb b)
...
module Fsubst = struct
  ...
  (* -------------------------------------------------------------------- *)
  let qe_subst (s : f_subst) (qe : quantum_equality) =
    { qeg = qe.qeg;
      qel = qr_subst s qe.qel;
      qer = qr_subst s qe.qer; }
  ...
  let rec f_subst ~(tx : tx) (s : f_subst) (fp : form) : form =
    ...
    | FequivF ef ->
      ...
      let ef_pr = f_subst_ec ~tx s ef.ef_pr in
      let ef_po = f_subst_ec ~tx s ef.ef_po in
    ...
    | FequivS es ->
      ...
      let es_pr = f_subst_ec ~tx s es.es_pr in
      let es_po = f_subst_ec ~tx s es.es_po in
  ...
  (* ------------------------------------------------------------------ *)
  and f_subst_ec ~(tx : tx) (s : f_subst) { ec_f; ec_e } : equiv_cond =
    { ec_f = f_subst ~tx s ec_f; ec_e = qe_subst s ec_e }
