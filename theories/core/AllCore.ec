...
abstract theory Cost.
  ...
  schema cost_incr {n:int} (m:int) : cost[`|n| <= m : n + 1] =
    cost[true:n] + N (cincr m).

  schema cost_eqint {n1 n2:int} (m:int) :
    cost[`|n1| <= m /\ `|n2| <= m : n1 = n2] =
    cost[true:n1] + cost[true:n2] + N (ceqint m).

  schema cost_ltint {n1 n2:int} (m:int) :
    cost[`|n1| <= m /\ `|n2| <= m : n1 < n2] =
    cost[true:n1] + cost[true:n2] + N (cltint m).

  schema cost_leint {n1 n2:int} (m:int) :
    cost[`|n1| <= m /\ `|n2| <= m : n1 < n2] =
    cost[true:n1] + cost[true:n2] + N (cleint m).
